...
public class AndController {
	...
	public AndController(Model m, CalculatorApp app) {
		this.model = m;
		this.app = app;
	}
	
	// Determine what command the user has written, or if we need to send them an error
	public static Result binAnd(GBInstruction instruction, Calculator calc){
		Result result;
		...
		if(operand0 == null ||  operand1 == null){
			...
			int andResult = operand0.getOperandValue(calc) & operand1.getOperandValue(calc);
